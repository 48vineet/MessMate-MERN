                  ...
                  {analyticsData.charts.dailyRevenue
                    ...
                    .map((day, index) => {
                      const maxRevenue = Math.max(
                        ...analyticsData.charts.dailyRevenue.map(
                          (d) => d.revenue
                        )
                      );
                      const height =
              ...
              {analyticsData?.charts?.mealDistribution?.length > 0 ? (
                <div className="h-full flex flex-col justify-center space-y-4">
                  {analyticsData.charts.mealDistribution.map((meal, index) => {
                    const totalOrders =
                      analyticsData.charts.mealDistribution.reduce(
                        (sum, m) => sum + m.orders,
                        0
                      );
                    const percentage =
                      totalOrders > 0 ? (meal.orders / totalOrders) * 100 : 0;
                    const colors = [
                      "bg-blue-500",
                      "bg-green-500",
                      "bg-yellow-500",
              ...
              {analyticsData?.charts?.topMeals?.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Start creating menus to see performance data
                  </p>
                </div>
              )}
            </div>
