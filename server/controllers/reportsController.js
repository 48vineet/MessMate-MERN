  ...
  try {
    ...
    res.status(200).json({
      ...
      report: {
        ...
        data: reportData, // Include the actual report data
      },
    });
  } catch (error) {
...
exports.getReportsHistory = async (req, res) => {
  try {
    const reports = await Report.find({})
      .select("type dateRange status createdAt")
      .sort({ createdAt: -1 })
      .limit(50)
      .lean();
...
exports.downloadReport = async (req, res) => {
  try {
    ...
    if (format === "pdf") {
      ...
      // Add footer
      doc.moveDown(2);
      doc.fontSize(10).text("Generated by MessMate System", {
        align: "center",
        color: "gray",
      });
      doc.fontSize(10).text("For support, contact your system administrator", {
        align: "center",
        color: "gray",
      });
...
const generateUserReport = async (startDate, endDate) => {
  ...
  const userActivity = await Booking.aggregate([
    ...
    {
      $group: {
        _id: "$user", // correct field in Booking schema
        bookingCount: { $sum: 1 },
    ...
    },
    {
      $group: {
        ...
        activeUserCount: { $sum: 1 }, // number of unique users with bookings
      },
    },
  ]);
